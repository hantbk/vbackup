<template>
  <div class="dashboard-container">
    <span class="duration">Statistics Time: {{ dateformat(backupinfo.time) }} Duration: {{ backupinfo.duration }}</span>
    ...
    <el-row :gutter="40" class="panel-group">
      ...
      <el-col :xs="12" :sm="12" :lg="8" class="card-panel-col">
        <panel :start-val="1" :end-val="backupinfo.fileTotal" text="Number of Files" icon-color="icon-red"
               icon="el-icon-files"/>
      </el-col>
      <el-col :xs="12" :sm="12" :lg="8" class="card-panel-col">
        <panel
  ...
  methods: {
    ...
    doGetAllRepoStats() {
      ...
      fetchDoGetAllRepoStats().then(res => {
        this.$notify.success(
          {
            message: 'Statistics are being calculated, please refresh the page later...',
            title: 'Notification'
          })
      }).finally(() => {
