...
// lockRepo -> lockRepo: Successfully acquired the lock
// lockRepo -> lockRepoExclusive: Failed to acquire the lock
// lockRepoExclusive -> lockRepoExclusive: Failed to acquire the lock
// lockRepoExclusive -> lockRepo: Failed to acquire the lock
...
func lockRepository(ctx context.Context, repo *repository.Repository, exclusive bool) (*restic.Lock, error) {
	...
	if err != nil {
		return nil, errors.WithMessage(err, "The repository is currently locked. Please wait for other operations to complete. If you are certain no other tasks are running, you can manually clear the lock. Lock information: ")
	}
