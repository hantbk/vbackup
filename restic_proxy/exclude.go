...
func isDirExcludedByFile(dir, tagFilename, header string) bool {
	...
	if err != nil {
		server.Logger().Warnf("could not access exclusion tagfile: %v", err)
		return false
	}
	...
	f, err := os.Open(tf)
	if err != nil {
		server.Logger().Warnf("could not open exclusion tagfile: %v", err)
		return false
	}
	defer func() {
		_ = f.Close()
	...
	if err == io.EOF {
		server.Logger().Warnf("invalid (too short) signature in exclusion tagfile %q\n", tf)
		return false
	}
	if err != nil {
		server.Logger().Warnf("could not read signature from exclusion tagfile %q: %v\n", tf, err)
		return false
	}
	if !bytes.Equal(buf, []byte(header)) {
		server.Logger().Warnf("invalid signature in exclusion tagfile %q\n", tf)
		return false
	}
	return true
}
...
func rejectByDevice(samples []string) (RejectFunc, error) {
	...
	return func(item string, fi os.FileInfo) bool {
		...
		// special case: make sure we keep mount points (directories which
