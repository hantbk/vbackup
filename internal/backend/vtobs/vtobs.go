...
import (
	...
	"github.com/hantbk/vbackup/pkg/restic_source/rinternal/backend/layout"
...
func open(ctx context.Context, cfg Config, rt http.RoundTripper) (*VtObs, error) {
...
func (v *VtObs) IsNotExist(err error) bool {
	var e minio.ErrorResponse
	return errors.As(err, &e) && e.Code == "NoSuchKey"
}

// Join combines path components with slashes.
func (v *VtObs) Join(p ...string) string {
	return path.Join(p...)
}
...
func (v *VtObs) ReadDir(ctx context.Context, dir string) (list []os.FileInfo, err error) {
	...
	debug.Log("using ListObjectsV1(%v)", v.cfg.ListObjectsV1)

	for obj := range v.client.ListObjects(ctx, v.cfg.Bucket, minio.ListObjectsOptions{
		Prefix:    dir,
		Recursive: false,
		UseV1:     v.cfg.ListObjectsV1,
	}) {
		if obj.Err != nil {
			return nil, err
		}
...
func (v *VtObs) Connections() uint {
	return v.cfg.Connections
}

// Location returns this backend's location (the bucket name).
func (v *VtObs) Location() string {
	return v.Join(v.cfg.Bucket, v.cfg.Prefix)
}

// Hasher may return a hash function for calculating a content hash for the backend
func (v *VtObs) Hasher() hash.Hash {
	return nil
}

// HasAtomicReplace returns whether Save() can atomically replace files
func (v *VtObs) HasAtomicReplace() bool {
	return true
}

// Path returns the path in the bucket that is used for this backend.
func (v *VtObs) Path() string {
	return v.cfg.Prefix
}

// Save stores data in the backend at the handle.
func (v *VtObs) Save(ctx context.Context, h restic.Handle, rd restic.RewindReader) error {
	objName := v.Filename(h)

	opts := minio.PutObjectOptions{StorageClass: v.cfg.StorageClass}
	...
	info, err := v.client.PutObject(ctx, v.cfg.Bucket, objName, io.NopCloser(rd), int64(rd.Length()), opts)
...
// given offset.
func (v *VtObs) Load(ctx context.Context, h restic.Handle, length int, offset int64, fn func(rd io.Reader) error) error {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()

	return backend.DefaultLoad(ctx, h, length, offset, v.openReader, fn)
}

func (v *VtObs) openReader(ctx context.Context, h restic.Handle, length int, offset int64) (io.ReadCloser, error) {
	objName := v.Filename(h)
	...
	coreClient := minio.Core{Client: v.client}
	rd, _, _, err := coreClient.GetObject(ctx, v.cfg.Bucket, objName, opts)
	if err != nil {
		return nil, err
	}
...
func (v *VtObs) Stat(ctx context.Context, h restic.Handle) (bi restic.FileInfo, err error) {
	objName := v.Filename(h)
	var obj *minio.Object

	opts := minio.GetObjectOptions{}

	obj, err = v.client.GetObject(ctx, v.cfg.Bucket, objName, opts)
	if err != nil {
...
func (v *VtObs) Remove(ctx context.Context, h restic.Handle) error {
	objName := v.Filename(h)

	err := v.client.RemoveObject(ctx, v.cfg.Bucket, objName, minio.RemoveObjectOptions{})
...
func (v *VtObs) List(ctx context.Context, t restic.FileType, fn func(restic.FileInfo) error) error {
	prefix, recursive := v.Basedir(t)
	...
	listresp := v.client.ListObjects(ctx, v.cfg.Bucket, minio.ListObjectsOptions{
		Prefix:    prefix,
		Recursive: recursive,
		UseV1:     v.cfg.ListObjectsV1,
	})
...
func (v *VtObs) Delete(ctx context.Context) error {
	return backend.DefaultDelete(ctx, v)
}

// Close does nothing
func (v *VtObs) Close() error { return nil }

// Rename moves a file based on the new layout l.
func (v *VtObs) Rename(ctx context.Context, h restic.Handle, l layout.Layout) error {
	debug.Log("Rename %v to %v", h, l)
	oldname := v.Filename(h)
	...
	_, err := v.client.CopyObject(ctx, dst, src)
	if err != nil && v.IsNotExist(err) {
	...
	return v.client.RemoveObject(ctx, v.cfg.Bucket, oldname, minio.RemoveObjectOptions{})
}
