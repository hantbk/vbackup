<template>
  <div class="app-container">
    <div class="handle-search">
      <el-form :model="listQuery" inline @submit.native.prevent>
        <el-form-item label="Operator">
          <el-input v-model="listQuery.operator" placeholder="Operator" class="filter-item" clearable/>
        </el-form-item>
        <el-form-item label="Action">
          <el-select v-model="listQuery.operation" class="handle-select mr5" clearable placeholder="Please select">
            <el-option
              v-for="(item, index) in [{value: '', name: 'All'}].concat(operationList)"
              :key="index"
              :label="item.name"
              :value="item.value"
            />
        ...
        <el-form-item label="Resource">
          <el-select v-model="listQuery.url" class="handle-select mr5" clearable placeholder="Please select">
            <el-option
              v-for="(item, index) in [{value: '', name: 'All'}].concat(resourceList)"
              :key="index"
              :label="item.name"
              :value="item.value"
            />
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="operator" align="center" label="Operator"/>
      <el-table-column prop="operation" align="center" label="Action">
      ...
      <el-table-column prop="url" align="center" label="Resource">
        <template slot-scope="{row}">
          {{ filterResource(row.url).name }}
        </template>
      </el-table-column>
      <el-table-column prop="url" align="center" label="Address"/>
      <el-table-column prop="data" align="center" label="Data">
        <template slot-scope="{row}">
    ...
    <pagination
      ...
      @pagination="getList"
    />
    <el-dialog title="Data Details" :visible.sync="dialogFormVisible" top="5vh">
...
export default {
  ...
  data() {
    return {
      operationList: [
        {name: 'Add', value: 'post', color: 'success'},
        {name: 'Edit', value: 'put', color: 'primary'},
        {name: 'Delete', value: 'delete', color: 'danger'},
      ],
      resourceList: [
        {name: 'Login', value: 'login'},
        {name: 'Repository', value: 'repository'},
        {name: 'Backup Plan', value: 'plan'},
        {name: 'User', value: 'user'},
        {name: 'Execute Backup', value: 'backup'},
        {name: 'Execute Data Restore', value: 'restore'},
        {name: 'Cleanup Policy', value: 'policy'},
        {name: 'Data Maintenance', value: 'restic'}
      ],
      listLoading: false,
      listQuery: {
        operator: '',
