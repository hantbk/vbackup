...
import (
	...
	"github.com/hantbk/vbackup/pkg/utils/docker"
...
func (e *BackupServer) setUpLogger() {
	...
	logfile, err := fs.OpenFile(path.Join(logpath, string(filepath.Separator), "vbackup.log"), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
		e.logger.Error(err)
	}
...
func (e *BackupServer) setUpPprofHandler() {
	...
	p := pprof.New()
	e.rootRoute.Any("/debug/pprof", iris.FromStd(p))
	e.rootRoute.Any("/debug/pprof/{action:path}", iris.FromStd(p))
}
